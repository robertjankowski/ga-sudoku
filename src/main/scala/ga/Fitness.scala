...
object Fitness {
   ...
   */
  def calculateFitness(grid: Grid, solution: Grid): Double = {
    val x = checkRows(grid) + checkColumns(grid) + checkSquares(grid) + compare(grid, solution)
    1.0 / x
  }
  ...
  private def checkSquares(grid: Grid) = {
    val boxes = Sudoku.createBoxes(grid)
    checkRows(boxes)
  }
